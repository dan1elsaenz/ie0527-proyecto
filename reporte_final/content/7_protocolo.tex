...
La Tabla \ref{tab:shockburst} muestra la estructura de bytes que utiliza este protocolo.
...
\begin{itemize}
    ...
    En la implementación, este paquete ocupa $8$ bytes con la estructura mostrada en la Tabla \ref{tab:start}.
    Además del campo \texttt{Tipo}, transporta el número total de bloques que se enviarán, la frecuencia de muestreo, la resolución en bits, el número de canales y el códec utilizado.
    El total de bloques viaja únicamente en el \texttt{START}, dado que el transmisor lo conoce antes de iniciar el envío, pues graba y fragmenta todo el audio previamente.
    El campo \texttt{Códec} permite al receptor seleccionar automáticamente la decodificación correcta y se codifica como \texttt{0x00} para PCM lineal, \texttt{0x01} para $\mu$-law (como extensión sugerida) y \texttt{0x02} para ADPCM.

\begin{table}[htb]
    \centering
    \caption{Estructura del paquete \texttt{START}.}
    \label{tab:start}
    \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        Tipo & Bloques & $f_s$ & Bits & Canales & Códec \\
        \SI{1}{\byte} & \SI{2}{\byte} & \SI{2}{\byte} & \SI{1}{\byte} & \SI{1}{\byte} & \SI{1}{\byte} \\
        \hline
    \end{tabular}
\end{table}

    \item \texttt{DATA:} Contiene los datos de audio con una capacidad máxima de \SI{32}{\byte} por paquete.
    Con el fin de monitorear la correcta recepción de los datos, se reservan algunos bytes para información de control, distribuyendo los \SI{32}{\byte} del \texttt{Payload} como se muestra en la Tabla \ref{tab:payload}.
...
\end{table}
    ...
    Dado que existen únicamente tres tipos de mensajes, \SI{1}{\byte} es suficiente para codificarlos (\texttt{START} = \texttt{0x01}, \texttt{DATA} = \texttt{0x02} y \texttt{END} = \texttt{0x03}).
